...
class 1ClickApplicationsApi(object):
    ...
    def __init__(self, api_client=None):
        ...
        self.install_kubernetes_endpoint = _Endpoint(
            settings={
                'response_type': (InlineResponse200,),
                'auth': [
                    'bearer_auth'
                ],
                'endpoint_path': '/v2/1-clicks/kubernetes',
    ...
    def install_kubernetes(
        ...
        **kwargs
        ...
        kwargs['model1_click_create'] = \
            model1_click_create
        return self.install_kubernetes_endpoint.call_with_http_info(**kwargs)

    def list(
        self,
        **kwargs
    ):
        """List 1-Click Applications  # noqa: E501

        To list all available 1-Click applications, send a GET request to `/v2/1-clicks`. The `type` may be provided as query paramater in order to restrict results to a certain type of 1-Click, for example: `/v2/1-clicks?type=droplet`. Current supported types are `kubernetes` and `droplet`.  The response will be a JSON object with a key called `1_clicks`. This will be set to an array of 1-Click application data, each of which will contain the the slug and type for the 1-Click.   # noqa: E501
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
